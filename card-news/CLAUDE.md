...
### Step 2.5: 리서치 검증 (하이브리드 모드)

리서치 결과의 정확성과 완성도를 팀 에이전트가 교차 검증합니다.

**두 가지 실행 방식을 지원합니다:**

#### 방식 A: Task 병렬 실행 (기본, 자동화)

2개의 Task 에이전트를 **병렬**로 실행한 뒤, 오케스트레이터가 종합
...
claude --agents fact-checker,supplementary-researcher \
  ...
  팩트체커는 통계 출처를 확인하고, 보완 리서처는 빠진 정보를 찾으세요. \
  서로 토론하여 최종 검증 결과를 합의하세요."
```

**장점**: 실시간 교차 검증, 더 깊은 협업
**단점**: 수동 실행, 토큰 비용 증가

---

**통과 기준 (공통)**: 모든 통계/수치가 `확인됨` 상태이고, 핵심 포인트가 5개 이상일 때
...
claude --agents hook-expert,copy-editor \
  ...
  후킹 점수 7점 이상 + 주요 지적사항 0건일 때 합의하고, \
  그렇지 않으면 구체적 수정안을 제시하세요."
```
...
4. 합의 도달 시 최종 평가 확정, 합의 실패 시 재토론

**장점**: 실시간 대화, 동적 합의, 더 깊은 협업
...
**통과 기준 (공통)**: 후킹 점수 7점 이상 + 카피 에디터의 주요 지적사항 0건
...
## Agent Teams 고급 사용법 (실험적)

> ⚠️ **요구사항**: `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` 환경변수 설정 필요 (사용자 설정에서 이미 활성화됨)

Agent Teams를 사용하면 여러 에이전트가 실시간으로 대화하며 협업할 수 있습니다.

### 활성화 확인

```bash
# settings.json 확인
cat ~/.config/claude-code/settings.json | grep AGENT_TEAMS
```
...
```bash
# 팀 시작
claude --agents agent1,agent2 "작업 설명"

# 예시: 리서치 검증
claude --agents fact-checker,supplementary-researcher \
  "workspace/research.md를 검증하세요"
```
...
팀이 실행되면 tmux split 화면으로 각 에이전트의 작업을 실시간 확인할 수 있습니다.

```
┌─────────────────────────────────┬─────────────────────────────────┐
│ fact-checker (팩트체커)         │ supplementary-researcher        │
│                                 │ (보완 리서처)                   │
│ 통계 출처 확인 중...            │ 추가 리서치 진행 중...          │
│                                 │                                 │
│ ✅ 글로벌 시장 $1.84조 확인됨   │ 💡 리세일 시장 데이터 발견      │
│ ❌ 스포츠 의류 수치 불일치      │ 📊 AI 임팩트 $1,500억 추가      │
└─────────────────────────────────┴─────────────────────────────────┘
```

### Best Practices

1. **파일 충돌 방지**: 각 에이전트가 다른 파일에 작업하도록 역할 분담
2. **명확한 작업 지시**: 각 에이전트의 임무를 구체적으로 명시
3. **합의 기준 제시**: "후킹 점수 7점 이상일 때 합의" 같은 명확한 기준
4. **타임아웃 설정**: 장시간 토론 방지를 위해 최대 시간 제한

### 제한사항

- **토큰 비용**: 여러 에이전트 동시 실행으로 비용 증가
- **타이밍 제어**: 자동화 파이프라인에 통합 어려움
- **권한 프롬프트**: 각 에이전트별로 권한 승인 필요할 수 있음
- **파일 잠금**: 같은 파일 동시 수정 시 충돌 가능
