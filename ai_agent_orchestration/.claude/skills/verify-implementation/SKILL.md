...
| # | 스킬 | 설명 |
|---|------|------|
| 1 | `verify-pipeline-flow` | 파이프라인 에이전트 의존성 위반과 컨텍스트 흐름 규칙 검사 |
| 2 | `verify-agent-structure` | 에이전트 파일 구조와 export 패턴 검사 |
| 3 | `verify-typescript-quality` | TypeScript 타입 품질과 strict 모드 준수 여부 검사 |
| 4 | `verify-prompt-conventions` | 시스템 프롬프트 파일 컨벤션과 일관성 검사 |
| 5 | `verify-output-coverage` | 파이프라인 산출물 저장 커버리지와 파일 생성 패턴 검사 |
| 6 | `verify-api-efficiency` | API 호출 방식 정확성과 비용·속도 효율성 검사 |
| 7 | `verify-security` | API 키 노출 위험과 보안 취약점 검사 |
...
| 검증 스킬 | 상태 | 이슈 수 |
|-----------|------|---------|
| verify-pipeline-flow | PASS / X개 이슈 | N |
| verify-agent-structure | PASS / X개 이슈 | N |
| verify-typescript-quality | PASS / X개 이슈 | N |
| verify-prompt-conventions | PASS / X개 이슈 | N |
| verify-output-coverage | PASS / X개 이슈 | N |
| verify-api-efficiency | PASS / X개 이슈 | N |
| verify-security | PASS / X개 이슈 | N |
...
- verify-pipeline-flow: 파이프라인 흐름 정상
- verify-agent-structure: 에이전트 구조 규칙 준수
- verify-typescript-quality: 타입 품질 양호
- verify-prompt-conventions: 프롬프트 컨벤션 준수
- verify-output-coverage: 산출물 저장 정상
- verify-api-efficiency: API 호출 효율적
- verify-security: 보안 취약점 없음
